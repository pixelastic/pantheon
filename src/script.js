  ...
  },
  {
    ...
    options: {
      ...
      templates: {
        text(options) {
          const poweredByUrl =
            'https://www.algolia.com/?utm_source=instantsearch.js&utm_medium=website&utm_content=gamemaster.pixelastic.com/gods&utm_campaign=poweredby';
          const suffix = `thanks to <a class="ais-Stats-link" href="${poweredByUrl}" target="_blank">Algolia</a>`;
          const { query, nbHits } = options;
          if (!query) {
            return `${nbHits} gods indexed, ${suffix}`;
          }
          return `${nbHits} gods found, ${suffix}`;
        },
      },
    },
  },
