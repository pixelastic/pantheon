...
module.exports = {
   ...
   **/
  alignment(page) {
    const rawAlignment = this.infobox(page).alignment;
    let alignment;

    // Some deities have changed alignment since their creation; we're only
    // interested in the modern worship
    if (_.includes(rawAlignment, 'Modern worship')) {
      alignment = _.chain(rawAlignment)
        .split('Modern worship')
        .at(1)
        .words()
        .slice(0, 2)
        .join(' ')
      ...
      alignment = _.startCase(rawAlignment);
    }

    // If the alignment is not one of the 9 valid ones, we fallback to "Other"
    return _.includes(dataAlignment, alignment) ? alignment : 'Other';
  },
  /**
  ...
  domains(page) {
    ...
    return (
      _.chain(`${domains1E}, ${domains2E}`)
        // Some deities have "alternative" domains, we still include theme
        .replace('Alternative:', ',')
        .replace('Alternate:', ',')
        .split(',')
        .map((domain) => {
